...
import { Mail, Phone, Network, List, ClipboardList } from 'lucide-react';
...
function OrgChartNode({ roleKey, roles, peopleByRole }: { roleKey: RoleKey; roles: Role[]; peopleByRole: Record<string, DirectoryPerson[]> }) {
  ...
  return (
    <div className="flex flex-col items-center">
      <Link
        href={`/team/${roleKey}`}
        className={`bg-surface-alt border ${accent.border} rounded-lg px-3 py-2 min-w-[160px] text-center hover:bg-elevated transition-colors`}
      >
  ...
  return (
    <div>
      ...
      {directoryQuery.data && view === 'chart' && topRole && (
        <>
          <div className="text-[11px] text-faint mb-2">Click anyone to see their task board.</div>
