...
import { X, Check, Calendar, AlertTriangle, Pencil, Flag } from 'lucide-react';
...
import { useMarkTaskRead } from '@/hooks/useMarkTaskRead';
...
export function TaskModal({
  ...
  const { data: commentsData, isLoading: commentsLoading, error: commentsError } = useTaskComments(task.id);
  ...
  return (
    <>
        ...
        >
          ...
          <div className="px-5 py-4">
            ...
            <div className="border-t border-border-soft pt-3.5 mt-3">
              ...
              {commentsLoading && <div className="text-xs text-faint">Loading comments…</div>}
              {commentsError && <div className="text-xs text-danger">Failed to load comments.</div>}
              {commentsData?.comments?.length === 0 && <div className="text-xs text-faint">No comments yet.</div>}
              {commentsData?.comments?.map((c: any) => (
                <div
                  key={c.id}
                  className={`bg-surface-alt border border-border-soft rounded-lg px-3 py-2.5 mb-2 ${c.isNew ? 'border-l-2 border-l-gold bg-gold-soft' : ''}`}
                >
                  <div className="flex justify-between mb-1">
                    <span className="text-[11.5px] font-semibold">
                      {c.author_name}{' '}
      ...
      {showAbuse && <AbuseReportModal taskId={task.id} onClose={() => setShowAbuse(false)} />}
    </>
  );
}